...
from utils import SR, change_pwd, download_nchlt

def load_nchlt_2(dataset_name, only_af = False, only_xh = False, write_audio = False):
    if (only_xh):
        if (only_af):
    ...
    if only_af:
        ...
        train_set, val_set = get_data_entries_nchlt(AF_PATH, af_sentences)
    elif only_xh:
        ...
        train_set, val_set = get_data_entries_nchlt(XH_PATH, xh_sentences)
    else:
        af_sentences = get_sentences_nchlt(os.path.join(AF_PATH, "transcriptions", "nchlt_afr.trn.xml"))
        af_sentences.update(get_sentences_nchlt(os.path.join(AF_PATH, "transcriptions", "nchlt_afr.tst.xml")))
        xh_sentences = get_sentences_nchlt(os.path.join(XH_PATH, "transcriptions", "nchlt_xho.trn.xml"))
        xh_sentences.update(get_sentences_nchlt(os.path.join(XH_PATH, "transcriptions", "nchlt_xho.tst.xml")))
        af_train, af_val = get_data_entries_nchlt(AF_PATH, af_sentences)
        xh_train, xh_val = get_data_entries_nchlt(XH_PATH, xh_sentences)
        train_set, val_set = af_train + xh_train, af_val + xh_val
    ...
    csv_entries = []
    for data_entry in tqdm(train_set):
        ...
        csv_entries.append([dst_path, data_entry[1]])
    metadata = pd.DataFrame(csv_entries, columns=['path', 'duration'])
    metadata.to_csv(path_or_buf=os.path.join(dataset_name, "train.csv"), sep=",", index=False)

    csv_entries = []
    for data_entry in tqdm(val_set):
        src_path = data_entry[0]
        ...
        csv_entries.append([dst_path, data_entry[1]])
    metadata = pd.DataFrame(csv_entries, columns=['path', 'duration'])
    metadata.to_csv(path_or_buf=os.path.join(dataset_name, "validation.csv"), sep=",", index=False)
    ...
    for speaker_dir in tqdm(speaker_dirs):
        ...
        for file_name in files:
            ...
            if sentence is not None:
                data_entry = [file_path, librosa.get_duration(y=librosa.load(file_path)[0], sr=SR)]
...
if __name__ == "__main__":
    load_nchlt_2("nchlt_afr", only_af=True, write_audio=True)